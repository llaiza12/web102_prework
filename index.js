...
const totalAmount = GAMES_JSON.reduce(
    (acc, game) => {
...
const unfundedGamesArray = GAMES_JSON.filter ( (game) => {
    return game.goal > game.pledged;
});

const numberOfUnfundedGames = unfundedGamesArray.length;

// create a string that explains the number of unfunded games using the ternary operator
const displayStr = 
    `A total of $${totalAmount.toLocaleString('en-US')} has been raised for ${totalGames} games. Currently, ${numberOfUnfundedGames > 1 ?  `${numberOfUnfundedGames} games remain` : '1 game remains'} unfunded. We need your help to fund these amazing games!`;
